eval_err(grd::MinimaxGrid{T}, x) where {T} = begin
  n = grd.n
  params = merge_params(n, grd.coefs, grd.exponents)
...
"""
  merge_params(n, coeffs, exponents)

Merge the coefficients and exponents into a single array.
"""
function merge_params(n, coeffs::Array{T}, exponents::Array{T}) where {T}
  @assert n == length(exponents)
  @assert n == length(coeffs)
  params = Array{T}(undef, 2n)
  params[1:n] .= coeffs
  params[n+1:end] .= exponents
  return params
end
...
function phi(n::Int64, params::Array{T}, μ::Array{T}) where {T}
    ...
    return fvals[1:end-1] .+ fvals[2:end]
end
